...
func (s *Store) Upsert(userID, token, platform string) error {
	_, err := s.db.Writer().Exec(`INSERT INTO push_tokens (user_id, token, platform, created_at)
...
func (s *Store) ListAll() ([]Token, error) {
	rows, err := s.db.Reader().Query(`SELECT id, user_id, token, platform, created_at FROM push_tokens`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) DeleteByToken(token string) error {
	_, err := s.db.Writer().Exec(`DELETE FROM push_tokens WHERE token = ?`, token)
	return err
}

// DeleteByUser removes all push tokens for a user (used on logout).
func (s *Store) DeleteByUser(userID string) error {
	_, err := s.db.Writer().Exec(`DELETE FROM push_tokens WHERE user_id = ?`, userID)
	return err
}
