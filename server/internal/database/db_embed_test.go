...
func TestMigrationCreatesNodeEmbeddings(t *testing.T) {
	...
	var name string
	err = db.Reader().QueryRow("SELECT name FROM sqlite_master WHERE type='table' AND name='node_embeddings'").Scan(&name)
	if err != nil {
		t.Fatalf("node_embeddings table not found: %v", err)
	}

	// Verify pinned column exists on nodes.
	_, err = db.Writer().Exec("SELECT pinned FROM nodes LIMIT 0")
	if err != nil {
		t.Fatalf("pinned column missing: %v", err)
	}

	// Verify consolidated_into column exists on nodes.
	_, err = db.Writer().Exec("SELECT consolidated_into FROM nodes LIMIT 0")
	if err != nil {
...
func TestSystemSettingsAndContentLength(t *testing.T) {
	...
	_, err = db.Writer().Exec(`INSERT INTO system_settings (key, value) VALUES ('test_key', 'test_value')`)
	if err != nil {
		t.Fatalf("insert into system_settings: %v", err)
	}
	var val string
	err = db.Reader().QueryRow(`SELECT value FROM system_settings WHERE key = 'test_key'`).Scan(&val)
	if err != nil {
	...
	_, err = db.Writer().Exec(`INSERT INTO nodes (id, type, title) VALUES ('test-node', 'fact', 'test')`)
	if err != nil {
		t.Fatal(err)
	}
	var cl sql.NullInt64
	err = db.Reader().QueryRow(`SELECT content_length FROM nodes WHERE id = 'test-node'`).Scan(&cl)
	if err != nil {
