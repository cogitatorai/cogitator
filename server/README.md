...
**Memory**: Persistent knowledge graph with vector retrieval. The agent stores facts, preferences, and patterns using the `save_memory` tool. Memories are embedded as vectors and retrieved via cosine similarity with a recency boost during conversation. Critical facts can be pinned so they are always present in the agent's context. A background consolidation worker clusters related episodes into higher-level patterns as the graph grows. When no embedding model is available, retrieval falls back to LLM-based classification.
